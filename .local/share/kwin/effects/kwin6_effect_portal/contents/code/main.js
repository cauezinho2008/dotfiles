...
class BurnMyWindowsPortalEffect {
  constructor() {
    ...
    this.shader = effect.addFragmentShader(Effect.MapTexture, 'portal.frag');
  ...
  loadConfig() {
    ...
    effect.setUniform(this.shader, 'uRotationSpeed',
                      effect.readConfig('RotationSpeed', 1.0));
  ...
  slotWindowAdded(window) {
    ...
    if (!BurnMyWindowsPortalEffect.shouldAnimateWindow(window)) {
      return;
    }
    if (!window.visible) {
      return;
